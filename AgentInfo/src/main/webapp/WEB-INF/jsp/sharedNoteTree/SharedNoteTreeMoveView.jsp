...
<script>
	...
	function select() {
		...
		var sharedNoteTreeFullPath = node.data.key; // 선택 트리 풀 경로
		var sharedNoteTreeName = node.data.title; // 선택 트리
		$.ajax({
		    url: "<c:url value='/employee/sharedNoteTreeMove'/>",
		    type: "POST",
			data: {
					chkList : chkList,
	...
	function reqMemberView()
	{
		...
		$("#sharedNoteTreeName").val(title); //트리경로
		$("#sharedNoteTreeFullPath").val(path); // 트리 풀 경로
