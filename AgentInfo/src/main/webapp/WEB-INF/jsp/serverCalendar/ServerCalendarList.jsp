...
<html lang="en" class=" js flexbox flexboxlegacy canvas canvastext webgl no-touch geolocation postmessage websqldatabase indexeddb hashchange history draganddrop websockets rgba hsla multiplebgs backgroundsize borderimage borderradius boxshadow textshadow opacity cssanimations csscolumns cssgradients cssreflections csstransforms csstransforms3d csstransitions fontface generatedcontent video audio localstorage sessionstorage webworkers no-applicationcache svg inlinesvg smil svgclippaths">
									...
									<div class="row align-items-center">
									    <div class="col-md-8">
									        <div class="page-header-title" >
									            <h5 class="m-b-10">장비 대여 일정 관리</h5>
									            <p class="m-b-0">Equipment rental schedule management</p>
									        </div>
									    </div>
									    <div class="col-md-4">
									        <ul class="breadcrumb-title">
									            ...
									            <li class="breadcrumb-item"><a href="#!">장비 대여 일정 관리</a>
									            </li>
									        </ul>
									    </div>
									</div>
	                                ...
	                                <div class="page-wrapper">
	                                	<div class="ibox">
	                                		<div style="background: white; padding: 20px; box-shadow: 1px 1px 1px grey; width: 1627px;">
												  ...
												  <div class="box box-solid" id="dragEvent">
													...
													<div class="box-body">
														<div class="add-node external-events" style="height: 265px; margin-bottom: 5px;">
															<c:forEach var="item" items="${serverCalendarList}">
																<div class="fc-event fc-h-event fc-daygrid-event fc-daygrid-block-event" style="color: white; background-color:${item.serverCalendarListColor}; border: 1px ${item.serverCalendarListColor}">${item.serverCalendarListContents}</div>
															</c:forEach>
														</div>
														<div class="checkbox">
												...
												<div id='serverCalendar-container'>
													<div id="serverCalendarTrash" class="serverCalendar-trash" style="margin-left: 14.5%; margin-bottom: 5px;">
														<img  src="/AgentInfo/images/trash.png" style="width:25px">
													</div>
												  	<div id='serverCalendar'></div>
												</div>
	                                		</div>
	                     				</div>
	                                </div>
	...
	<script>
		var list = JSON.parse('${serverCalendar}'); // List 결과 (자바스크립트 객체)를 문자열로 변환하여 list에 담음
		var dataList = [];
		...
		for(var i in list){
			var obj = {};
			obj._id = list[i].serverCalendarKeyNum;
			obj.title = list[i].serverCalendarContents;
			obj.start = list[i].serverCalendarStart;
			if(list[i].serverCalendarEnd != null){
				obj.end = list[i].serverCalendarEnd;
			}
			obj.allDay = list[i].serverCalendarAllDay;
			obj.backgroundColor = list[i].serverCalendarColor;
			obj.borderColor = list[i].serverCalendarColor
			dataList.push(obj);
		}
		...
		$(function() {
			...
			$('#serverCalendar').fullCalendar({
				...
				drop: function (date, allDay) {
					...
					$.ajax({
						url: "<c:url value='/serverCalendar/insert'/>",
						type: 'post',
						data : {
							'serverCalendarStartDate' : copiedEventObject.start._d.getTime(),
							'serverCalendarContents' : copiedEventObject.title,
							'serverCalendarColor' : copiedEventObject.backgroundColor,
							'serverCalendarAllDay' : copiedEventObject.start._ambigTime
						},
						async: false,
						success : function(result) {
							copiedEventObject.serverCalendarKeyNum = result.result;
					...
					$('#serverCalendar').fullCalendar('renderEvent', copiedEventObject, true);
				...
				eventResize : function(event, delta, revertFunc, jsEvent, ui, view) {
					Swal.fire({
						...
						if (result.isConfirmed) {
							...
							$.ajax({
								type: 'post',
								url: "<c:url value='/serverCalendar/move'/>",
								data : {
									'serverCalendarKeyNum' : event._id,
									'serverCalendarStartDate' : event.start._d.getTime(),
									'serverCalendarEndDate' : event.end._d.getTime(),
									'serverCalendarAllDay' : event.allDay
								},
								async: false,
								success : function(result){
							...
							$('#serverCalendar').fullCalendar('render');
						} else {
							revertFunc();
						}
					});
						...
						if (result.isConfirmed) {
							$.ajax({
								...
								data : {
									'serverCalendarKeyNum' : event._id,
									'serverCalendarStartDate' : event.start._d.getTime(),
									'serverCalendarEndDate' : (event.end != null) ? event.end._d.getTime() : event.start._d.getTime(),
									'serverCalendarAllDay' : event.allDay									
								},
								async: false,
								success : function(result){
				...
				eventDragStop: function(event,jsEvent) {
					var trashEl = jQuery('#serverCalendarTrash');
					...
					if (jsEvent.pageX >= x1 && jsEvent.pageX<= x2 && jsEvent.pageY >= y1 && jsEvent.pageY <= y2) {
						Swal.fire({
							...
							if (result.isConfirmed) {
								...
								$.ajax({
									type: 'post',
									url: "<c:url value='/serverCalendar/delete'/>",
									data : {
										'serverCalendarKeyNum' : event._id
									},
									async: false,
				...
				eventClick: function(calEvent, jsEvent, view) {
					$.ajax({
					    type: 'POST',
					    url: "<c:url value='/serverCalendar/view'/>",
					    data: {"serverCalendarKeyNum":calEvent._id},
					    async: false,
					    success: function (data) {
							if(data.indexOf("<!DOCTYPE html>") != -1) 
								location.reload();
					    	$.modal(data, 'serverCalendar'); //modal창 호출
					    },
					    error: function(e) {
					        alert(e);
					    }
			...
			$('#add-new-event').click(function(e) {
				...
				$.ajax({
					url: "<c:url value='/serverCalendarList/insert'/>",
					type: 'post',
					data: {
						'serverCalendarListContents':val,
						'serverCalendarListColor':currColor
					},
					async: false,
					success: function(result) {
						if(result.result == "OK") {
