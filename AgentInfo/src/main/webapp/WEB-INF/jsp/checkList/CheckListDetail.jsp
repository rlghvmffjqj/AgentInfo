...
<html>
	...
	<body style="background: white; padding: 2%;">
		<c:if test="${checkListSettingDetail.checkListSettingDetailMethod eq null}">
			<div class='noneDetail'>테스트 방법이 작성 되지 않았습니다.</div>
		</c:if>
		<c:if test="${checkListSettingDetail.checkListSettingDetailMethod ne ''}">
			<div>${checkListSettingDetail.checkListSettingDetailMethod}</div>
		</c:if>
