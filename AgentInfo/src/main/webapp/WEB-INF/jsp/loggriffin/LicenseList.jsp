...
>
                  ...
                  <div class="row align-items-center">
                    ...
                    <div class="col-md-4">
                      <ul class="breadcrumb-title">
                        ...
                        <li class="breadcrumb-item"><a href="#!">LogGRIFFIN / tGRIFFIN 라이선스 발급</a></li>
                      </ul>
                    </div>
                  </div>
                </div>
                                  ...
                                  <td>
                                    <div class="work-toolbar">
                                      ...
                                      <div class="toolbar-group">
                                        ...
                                        <button class="btn2 btn-light2 myBtn" id="BtnDownload" title="선택한 테이블 행의 YML 파일을 다운로드합니다.">⬇ 다운로드</button>
                                        <button class="btn2 btn-light2 myBtn" id="BtnImport" title="YML 파일을 첨부하여 데이터를 추가합니다.">⬆ YML Import</button>
                                        <button class="btn2 btn-light2 myBtn" id="BtnExcelExport" onclick="doExportExec()" title="현재 조회된 데이터를 Excel로 Export합니다.">📤 Excel</button>
                                      </div>
                                      ...
                                      <div class="toolbar-group">
                                        ...
                                        <button class="btn2 btn-light2 myBtn" id="BtnRoute" title="라이선스 발급 설정 경로를 지정합니다.">⚙ 경로설정</button>
                                        <button class="btn2 btn-light2 myBtn" id="BtnMailSetting" title="라이선스 만료 메일발송 관련 설정을 등록합니다.">📧 메일설정</button>
                                      </div>
                                      ...
                                      <div class="toolbar-group">
                                        ...
                                        <button class="btn2 btn-light2 myBtn" onclick="selectColumns('#list', 'licenseList')">⚙ 컬럼선택</button>
                                      </div>
                                    </div>
                                  </td>
                                </tr>
  ...
  <script>
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              ...
              async: false,
              success: function (data) {
                if (data == "OK") Swal.fire("성공!", "제거 완료하였습니다.", "success");
                else Swal.fire("실패!", "제거 실패하였습니다.", "error");
                tableRefresh();
              },
    ...
    function licenseNumFormatter(value, options, row) {
      ...
      return '<button class="btn2 btn-outline-info-nomal myBtn" onClick="licenseNumber(' + "'" + logGriffinKeyNum + "'" + ')">라이선스 발급</button>';
    }
    ...
    function licenseNumber(logGriffinKeyNum) {
      $.ajax({
        ...
        success: function (data) {
          if (data == "FALSE") {
            Swal.fire("실패!", "라이선스 발급 Key가 존재하지 않습니다.", "error");
          } else {
            Swal.fire("라이선스 발급 Key!", data, "success");
          }
        },
        error: function (e) {
          Swal.fire("에러!", "에러가 발생하였습니다.", "error");
        },
      });
    }
      ...
      if (chkList.length === 0) {
        ...
        $.ajax({
          ...
          success: function (result) {
            if (result === "Empty") {
              Swal.fire("에러!", "YML 파일이 존재하지 않거나, <br>존재하지 않는 리스트가 포함되어 있습니다.", "error");
            } else if (chkList.length === 1) {
  ...
  <script>
    ...
    function individualMailSend(keyNum) {
      $.ajax({
        ...
        async: false,
        success: function (data) {
          if (data == "OK") Swal.fire("발송!", "Mail 발송하였습니다.", "success");
          else Swal.fire("실패!", "Mail 발송 실패하였습니다.", "error");
        },
        error: function (e) {
          Swal.fire("실패!", "에러가 발생하였습니다.", "error");
        },
      });
    }
