...
>
                  ...
                  <div class="row align-items-center">
                    ...
                    <div class="col-md-4">
                      <ul class="breadcrumb-title">
                        ...
                        <li class="breadcrumb-item"><a href="#!">이슈 목록</a></li>
                      </ul>
                    </div>
                  </div>
                </div>
                                ...
                                <tr>
                                  <td>
                                    <div class="work-toolbar">
                                      <div class="toolbar-title">🐞 이슈 관리</div>
                                      ...
                                      <sec:authorize access="hasAnyRole('ADMIN','QA')">
                                        ...
                                        <!-- 처리 -->
                                        <div class="toolbar-group">
                                          <div class="group-label">처리</div>

                                          <button class="btn2 btn-success myBtn" id="BtnComplete">✅ 처리완료</button>
                                        </div>
                                      </sec:authorize>
                                      ...
                                      <div class="toolbar-group">
                                        ...
                                        <button class="btn2 btn-light2 myBtn" onclick="selectColumns('#list', 'issueKeyNum')">⚙ 컬럼 선택</button>
                                        <!-- <button class="btn2 btn-outline-info-nomal myBtn" id="BtnCopy">복사</button> -->
                                        <!-- <button class="btn2 btn-outline-info-nomal myBtn" id="BtnMerge">병합</button> -->
                                      </div>
                                    </div>
                                  </td>
                                </tr>
  ...
  <script>
      ...
      if (chkList.length > 1 && chkList.length < 4) {
        $.ajax({
          ...
          success: function (data) {
            if (data == "OK") Swal.fire("성공!", "병합 완료하였습니다.", "success");
            else if (data == "NotMatch") Swal.fire("불일치!", "고객사, Title, 전달일자, TOSMS, TOSRF, PORTAL, JAVA, WAS 정보가 일치해야합니다.", "error");
            else Swal.fire("실패!", "별합 실패하였습니다.", "error");
            tableRefresh();
          },
    ...
    function urlFormatter(cellValue, options, rowdata, action) {
      ...
      return '<button class="btn2 btn-outline-info-nomal myBtn" onclick="urlOpen(' + "'" + cellValue + "'" + ');">Open</button>';
    }
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              ...
              async: false,
              success: function (data) {
                if (data == "OK") Swal.fire("성공!", "처리완료 처리하였습니다.", "success");
                else Swal.fire("실패!", "처리완료 처리에 실패하였습니다.", "error");
                tableRefresh();
              },
              error: function (error) {
                console.log(error);
