<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%> <%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%> <%@ taglib prefix="sec" uri="http://www.springframework.org/security/tags" %>
...
<div class="modal-body" style="width: 100%; height: 760px; overflow-y: scroll">
  <form id="modalForm" name="form" method="post" enctype="multipart/form-data">
    ...
    <div class="package-modal__header">
      <h3 class="package-modal__title">테스트 업무</h3>
      <span class="package-modal__subtitle">테스트 패키지 제품을 입력해주세요.</span>
    </div>
    <div class="package-modal__content">
      <div class="package-modal__column">
        <section class="package-card">
          <h3 class="package-card__title">
            <svg class="package-card__icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              ...
              <div style="display: flow-root">
                <div class="pading5Width33">
                  <div>
                    ...
                    <select class="form-control selectpicker selectForm" id="workManageCustomerView" name="workManageCustomerView" data-live-search="true" data-size="5" title="선택해주세요">
                      <option value=""></option>
                ...
                <div class="pading5Width33">
                  <div>
                    ...
                    <input type="text" id="workManageEngineerView" name="workManageEngineerView" class="form-control viewForm" value="${workManage.workManageEngineer}" placeholder="입력해주세요" />
                  </div>
                </div>
                <div class="pading5Width33">
                  <label class="labelFontSize">요청구분</label>
                  <div>
                    <select class="form-control selectpicker selectForm" id="workManageDivisionView" name="workManageDivisionView" data-size="5" data-actions-box="true">
        ...
        <section class="package-card">
          ...
          <!-- 추가 버튼 -->
          <div style="margin: -45px 0; float: right">
            <button type="button" id="addProductBtn" class="btn btn-outline-info-add" style="border-radius: 8px !important">+ 제품유형 추가</button>
            <button type="button" id="delProductBtn" class="btn btn-outline-info-del" style="border-radius: 8px !important">- 제품유형 삭제</button>
          </div>
          ...
          <div class="package-field">
            <div class="package-field__head">
              <div>
                ...
                <div>
                  <!-- 기본 1개 -->
                  <div class="packageItem" style="display: flow-root; border-bottom: 1px dashed #ccc; height: 90px">
                      ...
                      <div class="pading5Width33" style="width: 150px">
                        ...
                        <div class="fields-container" style="display: flex; gap: 15px; width: max-content">
                          ...
                          <div class="field-item" style="width: 150px; flex-shrink: 0">
                            <!-- 제품유형 선택 -->
                            <div>
                              <select class="form-control selectpicker selectForm productType" name="workManageProductTypeView[]" data-live-search="true" data-size="5" title="선택해주세요">
                                <option value=""></option>
            ...
            <div class="package-field__head">
              <div style="padding-bottom: 10px">
                ...
                <div style="margin: 10px 0; text-align: right">
                  <button type="button" id="addPackageBtn" class="btn btn-outline-info-add" style="border-radius: 8px !important">+ 패키지 추가</button>
                </div>
                ...
                <div id="packageArea">
                  ...
                  <div class="packageItem" style="display: flow-root; margin-bottom: 15px; border-bottom: 1px dashed #ccc; padding-bottom: 10px">
                    <div class="packageRow">
                      <div class="pading5Width33 packageHeight" style="width: 68%">
                        <div>
        ...
        <section class="package-card" style="padding-right: 0px">
          <h3 class="package-card__title">
            <svg class="package-card__icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              ...
              <div style="padding-top: 10px">
                ...
                <div class="pading5Width33">
                  ...
                  <div>
                    ...
                    <select class="form-control selectpicker selectForm" id="workManageProgressStatusView" name="workManageProgressStatusView" data-size="5" data-actions-box="true" <sec:authorize access="!hasRole('ADMIN')">disabled="disabled"</sec:authorize>>
                ...
                <div class="pading5Width33" style="width: 100%; height: 100%">
                  <label class="labelFontSize">상세 내용</label>
                  <textarea class="form-control" id="workManageDetailNoteView" name="workManageDetailNoteView" spellcheck="false" placeholder="이슈내용 또는 테스트 내용 상세히 입력 바랍니다." style="resize: none; border: 1px solid #d6dee9">${workManage.workManageDetailNote}</textarea>
                </div>
              </div>
            </div>
          </div>
...
<script>
  $(document).ready(function () {
    ...
    $("#addPackageBtn").on("click", function () {
      ...
      // 패키지명
      html += '    <div class="pading5Width33 packageHeight" style="width: 68%;">';
      html += "        <div>";
    ...
    // 패키지 선택 버튼 클릭 이벤트
    $(document)
      .off("click", ".package-upload-btn")
      .on("click", ".package-upload-btn", function () {
    ...
    $(document)
      .off("change", ".packageFileInput")
      .on("change", ".packageFileInput", function (e) {
    ...
    $(document).on("click", ".package-delete-btn", function () {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $(this).closest(".packageItem").remove();
        }
      });
    });
  });
  ...
  $("#insertBtn").click(function () {
    ...
    $.each(postData, function (key, value) {
      if (key === "workManagePackageFileNameView" || key === "workManagePackageFileNameView[]" || key === "workManagePackageFileView" || key === "workManagePackageFileView[]") {
    ...
    $(".packageFileNameInput").each(function (index) {
      formData.append("workManagePackageFileNameView[" + index + "]", $(this).val() || "");
    });
    ...
    $(".packageFileInput").each(function (index) {
      if (this.files && this.files.length > 0) {
        hasFile = true;
        formData.append("workManagePackageFileView[" + index + "]", this.files[0]);
      } else {
          ...
          xhr.upload.addEventListener(
            ...
            function (event) {
              if (event.lengthComputable) {
                var percentVal = Math.round((event.loaded / event.total) * 100);

                bar.css("width", percentVal + "%");
                percent.html(percentVal + "%");
                status.html(event.loaded.toLocaleString() + " / " + event.total.toLocaleString() + " bytes");
              }
            },
            false,
          );
  ...
  $(document)
    .off("click", ".packageDownLoadBtn")
    .on("click", ".packageDownLoadBtn", function () {
  ...
  $(function () {
    setPackageData("${workManage.workManagePackageName}", "${workManage.workManagePackageFileName}", "${workManage.workManagePackageSize}");
  ...
  function loadEditData(typeStr, countStr) {
    ...
    var typeArray = typeStr
      ? typeStr.split(",").map(function (item) {
          return item.trim();
        })
      : [];
    var countArray = countStr
      ? countStr.split(",").map(function (item) {
          return item.trim();
        })
      : [];
  ...
  function setPackageData(workManagePackageNameView, workManagePackageFileNameView, workManagePackageSizeView) {
    ...
    for (let i = 0; i < workManagePackageNameView.length; i++) {
      ...
      // 패키지명 세팅
      item.find("input[name='workManagePackageNameView[]']").val(workManagePackageNameView[i]);

      item.find("input[name='workManagePackageFileNameView[]']").val(workManagePackageFileNameView[i]);
  ...
  $(document).ready(function () {
    $("#addProductBtn").on("click", function () {
      // 0. 현재 화면에 생성된 field-item의 개수를 체크 (최대 5개 제한)
      var currentCount = $(".field-item").length;
      if (currentCount >= 5) {
        Swal.fire({
          icon: "error",
          title: "제한!",
          text: "최대 5개까지만 추가할 수 있습니다.",
        });
