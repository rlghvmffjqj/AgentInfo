	...
	<head>
	    ...
	    <script>
			$(document).ready(function(){
				...
				$("#list").jqGrid({
					url: "<c:url value='/licenseRequest'/>",
					mtype: 'POST',
					postData: formData,
					datatype: 'json',
					colNames:['Key','구분','고객사명','사업명','추가정보','시작일','만료일','MAC주소','제품유형','iGRIFFIN Agent 수량','TOS 5.0 Agent 수량','TOS 2.0 Agent 수량','DBMS 수량','Network 수량','AIX(OS) 수량','HPUX(OS) 수량','Solaris(OS) 수량','Linux(OS) 수량','Windows(OS) 수량','관리서버 OS','관리서버 DBMS','국가','제품버전','요청자'],
					colModel:[
